...
Bthickness     = PlateDimension[0]-3;  //thickness of the enclosure rim 
BFrontHeight   = 6;      //Height of frontside of the enclosure rim 
BBackHeight    = 3;      //Height of Backside of the enclosure rim 
T0Buffer       = 4;      //Additional Plate thickness buffer for T0 position.
T0FrontH       = BFrontHeight + T0Buffer; //Adjusted height for T0 
T0BackH        = BBackHeight + T0Buffer;  //Adjuted height for T0

tenting      = [0,12,0]; // tenting for enclusoure  
...
ColumnOrigin = [//[translation vec] [rotation vec1] [rotation vec2]
                ...
                [[  -34,     -unit/2,  0], [0, 0,   0], [ 0, 90,  0]], //INDEX 2 knuckle
                [[  -17,     -unit/2,  0], [0, 0,   0], [ 0, 90,  0]], //INDEX 3 knuckle
...
module BuildTopPlate(keyhole = false,  trackball = false, platethickness = 0)
{
  ...
  difference(){
    union(){//SwitchPlate
      ...
      hull(){
        modPlateT(Hulls = true, hullSides = [0,RIGHT,0], rows = R2);
        modPlateT(Hulls = true, hullSides = [0,LEFT,0],  rows = R1);
      }
      hull(){
        modPlateT(Hulls = true, hullSides = [TOP,RIGHT,0], rows = R2);
      ...
      hull(){
        ...
        modBoarderWidT(true, BACK,RIGHT, cutLenM,       0, BBackHeight,   [0,BACK,0], R2); 
      }
        modBoarderWidT(false,BACK,RIGHT, cutLenM,       0, BBackHeight,      [0,0,0], R2); 
      hull(){
        modBoarderWidT(true,FRONT,RIGHT, cutLenM,       0, BBackHeight,[0,FRONT,TOP], R1); 
        modBoarderWidT(true, BACK,RIGHT, cutLenM,       0, BBackHeight, [0,BACK,TOP], R2); 
        modBoarderWidT(true, BACK,RIGHT, cutLenM,       0, BBackHeight,    [0,0,TOP], R2);       
      }   
      hull(){
        modBoarderWidT(true, BACK,RIGHT, cutLenM,       0, BBackHeight,  [0,FRONT,0], R2); 
        modBoarderLenT(true,FRONT, LEFT,       0, cutLenM, BBackHeight,  [RIGHT,0,0], R2); 
      }
        modBoarderLenT(false,FRONT,LEFT,      0, cutLenM, BBackHeight,       [0,0,0], R2); 
      hull(){
        modBoarderWidT(true, BACK, LEFT, cutLenM,       0, BBackHeight,  [0,FRONT,0], R2); 
        modBoarderLenT(true,FRONT, LEFT,       0, cutLenM, BBackHeight,   [LEFT,0,0], R2); 
      }
        modBoarderWidT(false,BACK,LEFT, cutLenM,        0, BBackHeight,      [0,0,0], R2); 
      hull(){
        modBoarderWidT(true, BACK, LEFT, cutLenM,       0, BBackHeight,   [LEFT,0,0], R2);
      ...
      hull(){
         ...
         modBoarderWid(true, FRONT, LEFT, cutLenM,       0, BBackHeight,   [0,FRONT,TOP], R0, C1);
         modBoarderWid(true, FRONT, LEFT, cutLenM,       0, BBackHeight,   [LEFT,BACK,TOP], R0, C1);
         modBoarderLenT(true,FRONT,RIGHT,       0, cutLenM, BBackHeight,     [0,FRONT,0], R2); 
      ...
      hull(){
        modBoarderWidT(true,BACK, LEFT, cutLenM,       0, BBackHeight,  [0,FRONT,0], R2); 
        modBoarderLenT(true,FRONT,RIGHT,       0, cutLenM, BBackHeight,   [LEFT,0,0], R2); 
      ...
      hull(){
        ...
        modBoarderLenT(true,FRONT, LEFT, cutLen,       0,     T0BackH,       [TOP,0,0], R0, Buffer=T0Buffer); 
        modBoarderLenT(true,FRONT, LEFT, cutLen,       0,     T0BackH,[BACK,FRONT,BOTTOM], R0, Buffer=T0Buffer);
        modBoarderWidT(true,    0, LEFT,      0,       0,     T0BackH,     [TOP,FRONT,0], R0, Buffer=T0Buffer);
        
        modBoarderLenT(true,FRONT,RIGHT,      0, cutLenM, BBackHeight,[LEFT,FRONT,TOP], R2); 
        modBoarderWidT(true,BACK,  LEFT,cutLenM,       0, BBackHeight,[LEFT,FRONT,TOP], R2);
      }
    ...
    union(){     
      if(keyhole == true){
        ...
        PlaceOnThumb(2)rotate([0,0,-180])Keyhole(clipLength = cutLen, cutThickness = 2);    
 ...
 //Thumb Section
  ...
  hull(){
    transEnclose(){
      ...
      modBoarderWidT(true, BACK,RIGHT,cutLenM,       0,  BBackHeight, [BOTTOM,BACK,0], R2); 
      modBoarderWidT(true, BACK,RIGHT,cutLenM,       0,  BBackHeight,[BOTTOM,FRONT,0], R2); 
   }
   projectEnclose(){
      modBoarderWidT(true,FRONT,RIGHT,cutLenM,       0,  BBackHeight, [BOTTOM,FRONT,0], R1, [EScale,1,1]); 
      modBoarderWidT(true, BACK,RIGHT,cutLenM,       0,  BBackHeight,  [BOTTOM,BACK,0], R2, [EScale,1,1]); 
      modBoarderWidT(true, BACK,RIGHT,cutLenM,       0,  BBackHeight, [BOTTOM,FRONT,0], R2, [EScale,1,1]); 
   }  
  }  
  hull(){
    transEnclose(){
      modBoarderWidT(true, BACK,RIGHT, cutLenM,       0,  BBackHeight, [RIGHT,FRONT,0], R2); 
      modBoarderLenT(true,FRONT,RIGHT,       0, cutLenM,  BBackHeight, [RIGHT,FRONT,0], R2); 
    }
    projectEnclose(){
  ...
  hull(){
    transEnclose(){
      modBoarderLenT(true,FRONT,RIGHT,      0, cutLenM,  BBackHeight,     [RIGHT,0,0], R2); 
      modBoarderWid(true, BACK,  LEFT,  cutLen,       0, BFrontHeight, [0,BACK,BOTTOM], R1, C1,[RScale,1,1]);
      modBoarderWid(true, FRONT, LEFT, cutLenM,       0, BBackHeight, [0,FRONT,BOTTOM], R0, C1,[RScale,1,1]);
    }
    projectEnclose(){
...
//  difference(){
//  union(){
    rotate(tenting)translate([0,0,plateHeight])BuildTopPlate(keyhole = true, trackball = true, channel = false, platethickness =-1);
...
//    #rotate(tenting)translate([0,0,plateHeight])mockPCB(1, 3);
     ...
     rotate(tenting)translate([0,0,plateHeight])translate(trackOrigin)color("royalblue")sphere(d=40,$fn= 64);
//  }
//}
  //caps
//  rotate(tenting)translate([0,0,plateHeight]){
...
//    PlaceOnThumb(2)rotate([0,0,-180])Switch(colors = "Steelblue",clipLength = cutLen);
//  }
