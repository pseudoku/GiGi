...
cutLen         = 3.2;    //3.2scut length for clipped khail 
cutLenM        = cutLen+2; // fudging 
...
BFrontHeight   = 2;      //Height of frontside of the enclosure rim 
BBackHeight    = 2;      //Height of Backside of the enclosure rim 
...
tenting        = [0,9,0]; // tenting for enclusoure  
plateHeight    = 11.5;       // height adjustment for enclusure 
trackOrigin    = [-1,-22,4]; //trackball origin
trackTilt      = [10,0,0];    //angle for tilting trackpoint support and PCB
...
ColumnOrigin = [//[translation vec] [rotation vec1] [rotation vec2]
                ...
                [[  -30,     -unit/2,  0], [0, 0,   0], [ 0, 90,  0]], //INDEX 2 knuckle
                [[-12.5,     -unit/2,  0], [0, 0,   0], [ 0, 90,  0]], //INDEX 3 knuckle
                [[    0,           0,  0], [0, 0,   0], [ 0, 90,  0]], //MIDDLE knuckle
                [[   18,     -unit/2,  0], [0, 0,  -0], [ 0, 90,  0]], //RING knuckle
                [[ 25.0,   -unit*9/8,  0], [0, 0, -15], [ 0, 90,  0]], //PINKY 1 knuckle
                [[ 42.0, -unit*9/8-1, -3], [0, 0, -15], [ 0, 90,  0]], //PINKY 2 knuckle
                [[    4,           0,  4], [0, 0,   0], [ 0,  0,  0]]  //Thumb wrist origin
               ];
...
ThumbPosition = 
 [//[[thetaDist, thetaMed, thetaProx, phiProx][rotation angle][rotation angle2][translation vec], clipLen, clip orientation] 
    [[-32,-20, 6.5],[ 0,-130, 15]], //R0
    [[-36,-22, -14],[15, -60,  0]], //R1
    [[-43, -3,-9.9],[40, -60, -0]]  //R2 43
 ];
...
RowTrans     =[[    0,   .8,   .8,   .8,   .8,  .41,  .20,  0],  //R0
               [ 1.05, 1.98, 1.98, 1.95, 1.95, 1.60, 1.44,  1],  //R1s
...
module BuildTopPlate(keyhole = false,  trackball = false, platethickness = 0)
{
  ...
  difference(){
    union(){//SwitchPlate
      ...
      //Top side
      hull(){ //C1R0 Edge
        modBoarderLen(true,  BACK,RIGHT,cutLenM,        0, BBackHeight,       [LEFT,0,TOP], R0, C1);
        modBoarderWid(true, FRONT, LEFT,      0, cutLenM,  BBackHeight,       [0,BACK,TOP], R0, C1);   
      
        modBoarderLen(true,  BACK,RIGHT,cutLenM,        0,  BBackHeight,    [LEFT,0,BOTTOM], R0, C1, [1,RScale,1]);  
        modBoarderWid(true, FRONT, LEFT,      0,  cutLenM, BBackHeight,    [0,BACK,BOTTOM], R0, C1, [RScale,1,1]);     
      }
      hull(){
        modBoarderWid(true,  BACK, LEFT,      0, cutLenM, BFrontHeight,     [0,BACK,TOP], R1, C1);  
        modBoarderWid(true, FRONT, LEFT,      0, cutLenM,  BBackHeight,    [0,FRONT,TOP], R0, C1);  
        modBoarderWid(true, FRONT, LEFT,      0, cutLenM,  BBackHeight,     [0,BACK,TOP], R0, C1);   
        
        modBoarderWid(true,  BACK, LEFT,      0, cutLenM, BFrontHeight,  [0,BACK,BOTTOM], R1, C1, [RScale,1,1]);  
        modBoarderWid(true, FRONT, LEFT,      0, cutLenM,  BBackHeight, [0,FRONT,BOTTOM], R0, C1, [RScale,1,1]);  
        modBoarderWid(true, FRONT, LEFT,      0, cutLenM,  BBackHeight,  [0,BACK,BOTTOM], R0, C1, [RScale,1,1]); 
      }
      hull(){
        modBoarderWid(true,  BACK, LEFT,      0, cutLenM, BFrontHeight,        [0,0,TOP], R1, C1);   
        modBoarderWid(true,  BACK, LEFT,      0, cutLenM, BFrontHeight,     [0,0,BOTTOM], R1, C1, [RScale,1,1]);   
      }
      hull(){
        modBoarderWid(true,  BACK, LEFT,      0, cutLenM, BFrontHeight,    [0,FRONT,TOP], R1, C1);  
        modBoarderLen(true,FRONT,RIGHT,cutLenM,       0, BFrontHeight,     [LEFT,0,TOP], R1, C1);
      
        modBoarderWid(true,  BACK, LEFT,      0, cutLenM, BFrontHeight, [0,FRONT,BOTTOM], R1, C1, [RScale,1,1]);  
        modBoarderLen(true, FRONT,RIGHT,cutLenM,       0,  BFrontHeight, [LEFT,0,BOTTOM], R1, C1, [1,RScale,1]);
      }
      hull(){
        modBoarderLen(true, FRONT,RIGHT,cutLenM,       0, BFrontHeight,        [0,0,TOP], R1, C1);
        modBoarderLen(true, FRONT, LEFT,cutLenM,       0, BFrontHeight,        [0,0,TOP], R1, C2);
        modBoarderLen(true, FRONT,    0,       0, cutLenM, BFrontHeight,        [0,0,TOP], R1, C3);
      
        modBoarderLen(true, FRONT,RIGHT,cutLenM,       0, BFrontHeight,     [0,0,BOTTOM], R1, C1, [1,RScale,1]);
        modBoarderLen(true, FRONT, LEFT,cutLenM,       0, BFrontHeight,     [0,0,BOTTOM], R1, C2, [1,RScale,1]);
      ...
      hull(){
        ...
        modBoarderWid(true, BACK, RIGHT, cutLenM,       0, BBackHeight,       [0,0,0], R1, C4);
        modBoarderWid(true,FRONT, RIGHT, cutLenM,       0, BBackHeight,      [0,FRONT,0], R0, C4);
      ...
      //Bottom side
      hull(){  
        modBoarderLen(true, BACK,RIGHT, cutLenM,     0, BBackHeight,        [0,0,TOP], R0, C1);
        modBoarderLen(true, BACK,RIGHT, cutLenM,     0, BBackHeight,     [0,0,BOTTOM], R0, C1, [1,RScale,1]);
      }
      hull(){  
        modBoarderLen(true, BACK, LEFT,cutLenM,     0, BBackHeight,        [0,0,TOP], R0, C2);
        modBoarderLen(true, BACK, LEFT,cutLenM,     0, BBackHeight,     [0,0,BOTTOM], R0, C2, [1,RScale,1]);
      }
      hull(){ 
        modBoarderLen(true, BACK, LEFT,cutLenM,     0, BBackHeight,        [0,0,TOP], R0, C2);
      ...
      hull(){
        ...
        modBoarderWidT(true, BACK,RIGHT,       0, cutLenM, BBackHeight,    [0,0,TOP], R2);       
      }   
      hull(){
      ...
      hull(){
         modBoarderWid(true,  BACK, LEFT,       0, cutLenM,BFrontHeight,    [0,BACK,TOP], R1, C1);
         modBoarderWid(true, FRONT, LEFT,       0, cutLenM,BBackHeight,   [0,FRONT,TOP], R0, C1);
         modBoarderWid(false,FRONT, LEFT,       0, cutLenM, BBackHeight, [LEFT,BACK,TOP], R0, C1);
      ...
      hull(){
        ...
        modBoarderLenT(true,FRONT, LEFT,       0,       0, BBackHeight,      [LEFT,FRONT,0], R2);
      ...
      hull(){
        modBoarderWid(true, FRONT, LEFT,      0, cutLenM,  BBackHeight,       [LEFT,BACK,TOP], R0, C1);  
        modBoarderLen( true, BACK, RIGHT, cutLenM,     0,  BBackHeight, [LEFT,BACK,TOP], R0, C1);
//        modBoarderWid(true, FRONT, LEFT,cutLenM,       0, BBackHeight,   [LEFT,BACK,0], R0, C1);  
        modBoarderWidT(true,BACK,  LEFT,      0,       0, BBackHeight,    [LEFT,FRONT,0], R2);   
        modBoarderLenT(true,FRONT, LEFT, cutLen,      0 ,    T0BackH,      [0,FRONT,0], R0, Buffer=T0Buffer); 
      }
      hull(){
        modBoarderLen(true,  BACK,RIGHT,cutLenM,        0, BBackHeight,       [LEFT,0,TOP], R0, C1);
        modBoarderWid(true, FRONT, LEFT,      0, cutLenM,  BBackHeight,       [0,BACK,TOP], R0, C1);  
        ...
        modBoarderLenT(true,FRONT, LEFT, cutLen,      0 ,    T0BackH,      [BOTTOM,0,0], R0, Buffer=T0Buffer);
        modPlateWidT(Hulls = true, hullSides = [TOP,RIGHT,0], rows = R2);
        modPlateWidT(Hulls = true, hullSides = [TOP,LEFT,0],  rows = R1); 
      }
      ...
      if(trackball == true){
        hull(){  
          ...
          modBoarderLenT(true,FRONT, LEFT, cutLen,       0,      T0BackH, [RIGHT,FRONT,0], R0, Buffer=T0Buffer);
          modBoarderLenT(true, BACK, LEFT, cutLen,       0,      T0BackH,  [RIGHT,BACK,0], R0, Buffer=T0Buffer);
        }   
      }
    }
    // !!!CUTS
    union(){     
      if(keyhole == true){
        ...
        BuildRmCn(R0, C1)rotate([0,0,-90])Keyhole(clipLength = Clipped[R0][C1], cutThickness = 0);       
        BuildRmCn(R1, C1)rotate([0,0,-90])Keyhole(clipLength = Clipped[R1][C1]);
...
module BuildEnclosure(platethickness = 0, trackball = false, edgeonly = false, Jacks = false)
{ 
  ...
  difference(){
    union(){
      //TOP side
      hull(){
        transEnclose()modBoarderWid(true, BACK, LEFT,       0,  cutLenM,BFrontHeight, [0,0,BOTTOM],R1, C1, [RScale,1,1]);   
        projectEnclose()modBoarderWid(true, BACK, LEFT,      0, cutLenM, BFrontHeight, [0,0,BOTTOM],R1, C1, [EScale,1,1]);
      }
      hull(){
        transEnclose(){
          modBoarderWid(true,  BACK, LEFT,        0, cutLenM, BFrontHeight, [0,FRONT,BOTTOM], R1, C1, [RScale,1,1]);  
        ...
        projectEnclose(){
          modBoarderWid(true,  BACK, LEFT,       0, cutLenM, BFrontHeight, [0,FRONT,BOTTOM], R1, C1, [EScale,1,1]);  
          modBoarderLen(true, FRONT,    0, cutLenM,       0, BFrontHeight,  [LEFT,0,BOTTOM], R1, C1, [1,EScale,1]);      
        }
      }
      hull(){
        transEnclose(){
...
//          modBoarderWid(true, FRONT, LEFT, cutLenM,       0,  BBackHeight,[0,FRONT,BOTTOM], R0, C1,[RScale,1,1]);
        ...
        projectEnclose(){
          modBoarderWidT(true, BACK,RIGHT,       0, cutLenM, BBackHeight, [BOTTOM,FRONT,0], R2, [EScale,1,1]); 
          modBoarderWid(true, BACK,  LEFT,       0, cutLenM, BFrontHeight, [0,BACK,BOTTOM], R1, C1,[EScale,1,1]);
...
module TrackBall(){
  difference(){
    union(){
     palm_track();
     rotate(PCBA)translate([0,0,-42/2])cube([30,22,8],center= true); // pcb housing
    }
...
//    sphere(d= 40.5); 
    ...
    //lower bearing
    for(i= [0:2])rotate([0,40,120*i-90])translate([40/2,0,0])sphere(r=rbearing); //upper bearing for tighter fit?
    for(i= [0:1])rotate([0,-10,180*i])translate([40/2,0,0])sphere(r=rbearing); //upper bearing for tighter fit?
...
//rotate([0,0,360*$t]){
  ...
  union(){
    ...
    BuildEnclosure(trackball = false, edgeonly = true, platethickness = -1);
//    color("royalblue")rotate([180,0,0])translate([-15,-20,-2])PCB();//PMW3360
...
//  rotate(tenting)translate([0,0,plateHeight])translate(trackOrigin)color("royalblue")sphere(d=40.5,$fn= 64);
}
  //caps
  rotate(tenting)translate([0,0,plateHeight]){
//    BuildSet2();
//    PlaceOnThumb(0)rotate([0,0,-90])Switch(colors = "Steelblue",clipLength = cutLen);
//    PlaceOnThumb(1)rotate([0,0,-90])Switch(colors = "Steelblue",clipLength = cutLen);
//    PlaceOnThumb(2)rotate([0,0,-90])Switch(colors = "Steelblue",clipLength = cutLen);
  }
  
//    rotate(tenting)translate([0,0,plateHeight])translate(trackOrigin)color("royalblue")sphere(d=34,$fn= 64);
